...
void EnvironmentLight::init() {
	...
	for (int x = 0; x < w; x++) {
		for (int y = 0; y < h; y++) {
			pdf_envmap[y*w+x] = (envMap->data[y*w+x].illum()*sin(PI*y/h))/sum;
		}
	} 
...
Spectrum EnvironmentLight::sample_L(const Vector3D& p, Vector3D* wi,
  ...
  // TODO: 3-2 Part 3 Tasks 2 and 3 (step 4)
	...
	// *wi = sample;
	// return bilerp(theta_phi_to_xy( dir_to_theta_phi( sample ) ));

	
	Vector2D t =  sampler_uniform2d.get_sample();
	int y = std::upper_bound(marginal_y, marginal_y+h, t.y) - marginal_y;
	int x = std::upper_bound(conds_y+y*w, conds_y + (y+1)*w, t.x) - conds_y - y*w;
