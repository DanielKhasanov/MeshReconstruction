...
Ray Camera::generate_ray(double x, double y) const {
  ...
  Vector3D d = (c2w*(Vector3D(bottomLeft.x + (topRight.x - bottomLeft.x)*x, bottomLeft.y + (topRight.y - bottomLeft.y)*y , -1.0))).unit() ;
  Ray r = Ray(pos, d);
  r.min_t = nClip;
  r.max_t = fClip;
...
Ray Camera::generate_ray_for_thin_lens(double x, double y, double rndR, double rndTheta) const {
    ...
    Vector3D d = (Vector3D(bottomLeft.x + (topRight.x - bottomLeft.x)*x, bottomLeft.y + (topRight.y - bottomLeft.y)*y , -1.0)).unit() ;
