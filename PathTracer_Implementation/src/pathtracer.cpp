...
void PathTracer::ball_pivot() {
  ...
  // build BVH //
  fprintf(stdout, "[PathTracer] Building Surface Using BPA from %lu primitives... ", primitives.size()); 
  fflush(stdout);
  timer.start();
  // bvh = new BVHAccel(primitives);
  timer.stop();
...
Spectrum PathTracer::estimate_direct_lighting(const Ray& r, const Intersection& isect) {
  ...
  for (SceneLight* light : scene->lights) {
    ...
    for (int _ = 0; _ < numSamples; _++) {
      ...
      const Ray shadow_r =  Ray(hit_p + EPS_D*wi, wi, (double) distToLight);
      if (!(bvh->intersect(shadow_r))) {
         L_light += (sample)*(isect.bsdf->f (w_out, w_in))*(w_in.z)*(1.0/pdf);
      }

    }
    L_out += L_light/((double) numSamples);
...
Spectrum PathTracer::estimate_indirect_lighting(const Ray& r, const Intersection& isect) {
  ...
  Vector3D wi;
  float pdf;
  Spectrum sample = isect.bsdf->sample_f(w_out, &wi, &pdf);
  if (pdf <= 0) {
    return Spectrum();
  }
  Vector3D w_in = w2o*wi;

  double probability = clamp(0.05 + 15.0*sample.illum(), 0.0, 1.0);
  if (coin_flip(probability)) { 

    Ray bounce = Ray(EPS_D*o2w*wi + hit_p, o2w*wi);
    bounce.depth = r.depth - 1;

    Spectrum ray = trace_ray(bounce, isect.bsdf->is_delta());

    return (sample)*(ray)*(abs_cos_theta(wi))*(1.0/(pdf*probability));
  }
...
Spectrum PathTracer::raytrace_pixel(size_t x, size_t y) {
  ...
  if (num_samples == 1) {
      ...
      scaled_x = (((double) x) + 0.5) / (double) sampleBuffer.w;
      scaled_y = (((double) y) + 0.5) / (double) sampleBuffer.h;
      Ray r = this->camera->generate_ray(scaled_x, scaled_y);
      r.depth = max_ray_depth;
      s = this->trace_ray(r, true); 
    ...
    for (int i = 0; i < num_samples; i++) {
        rnd = this->gridSampler->get_sample();
        scaled_x = (((double) x) + rnd.x) / (double) sampleBuffer.w;
        scaled_y = (((double) y) + rnd.y) / (double) sampleBuffer.h;

        Vector2D rnd = gridSampler->get_sample();
        Ray r = this->camera->generate_ray_for_thin_lens(scaled_x, scaled_y, rnd.x, rnd.y);

        r.depth = max_ray_depth;

        Spectrum sampled_ray = this->trace_ray(r, true);
        s+=sampled_ray;
        double xk = sampled_ray.illum();
        s1 += xk;
        s2 += xk*xk;
        if (i%samplesPerBatch == 0 && i != 0) {
          mu = s1/((double) i);
          sigsqr = (1.0/((double) i - 1.0))*(s2 - ((s1*s1) / ((double) i)) );
          II = 1.96*(sqrt(sigsqr)/( sqrt((double) i)) );
          if (II <= maxTolerance*mu) {
                s/= ((double) i);
