...
int main( int argc, char** argv ) {
  ...
  while ( (opt = getopt(argc, argv, "s:l:t:m:e:h:f:r:c:a:p:b:d:")) != -1 ) {  // for each option...
    switch ( opt ) {
    case 'f':
        write_to_file = true;
  ...
  if (sceneFilePath.substr(sceneFilePath.length()-4, 4) == ".dae")
  {
  ...
  else if (sceneFilePath.substr(sceneFilePath.length()-4, 4) == ".ply") {

      string sceneFile = sceneFilePath.substr(sceneFilePath.find_last_of('/')+1);
      sceneFile = sceneFile.substr(0,sceneFile.find(".ply"));
      config.pathtracer_filename = sceneFile;

    if ( Collada::PointCloudParser::load(sceneFilePath.c_str(), sceneInfo) < 0) {
      delete sceneInfo;
      return -1;
    }
  }
