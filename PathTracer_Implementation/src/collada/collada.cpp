...
SceneInfo* PointCloudParser::scene; // pointer to output scene description

Vector3D PointCloudParser::up; // scene up direction
Matrix4x4 PointCloudParser::transform; // current transformation

Vector3D point_from_string ( string point_string ) {

  Vector3D point;
  stringstream ss (point_string);
  ss >> point.x;
  ss >> point.y;
  ss >> point.z;
  return point;
...
int num_from_string ( string num_string ) {
  int num;
...
int PointCloudParser::load( const char* filename, SceneInfo* sceneInfo ) {
    ifstream in( filename );
    if ( !in.is_open() ) {
      return -1;
    } in.close();
    ...
    CameraNode.instance = camera;
    camera->id   = "DUMMY CAMERA ID";
    camera->name = "DUMMY CAMERA NAME";
    camera->type = Instance::CAMERA;
    camera->up_dir   = up;
    camera->view_dir = Vector3D(0,0,-1);
    CameraNode.instance = camera;
    scene->nodes.push_back(CameraNode);
    ...
    // light->id   = "DUMMY LIGHT ID";
    // light->name = "DUMMY LIGHT NAME";
    // light->type = LIGHT;
    // Color c;
    // c.r = 1;
    // c.g = 1;
    ...
    // light->color = c;
    // light->attenuation = 1;
    // LightNode.instance = light;
    // scene->nodes.push_back(LightNode);


    /*Create the Naive polygons for connecting sequentially*/
    Node polynode = Node();
    PolymeshInfo* polymesh = new PolymeshInfo();
    string line;

    /*Read in the file containing the number of elements followed by the vertex location*/
    ifstream plyfile (filename);
    if (plyfile.is_open())
    {

      /*Read in the number of vertices, compute the number of resulting polygons accordingly*/
      std::getline (plyfile,line);
      int num_vertices = num_from_string(line);
      int num_polygons = num_vertices/3 + 1;

      /*Allocate space for the vertices and the polygons*/
      polymesh->polygons.resize(num_polygons);
      vector<Vector3D> vertices;

      /*These are used for normalizing the ply file into a reasonable size
        as well as proper;y padding for our nearest neighbor solution*/
      double normalization = 0.0;
      int i = 0;
      while ( getline (plyfile,line) )
      {
        polymesh->polygons[i/3].vertex_indices.push_back(i);
        Vector3D v = point_from_string(line);

        /*In the case that there are not 3n vertices, we must pad the last one*/
        if ( (num_polygons - 1)%3 && (i == num_vertices - 1)) {
        ...
        /*Accrue the normalization factor*/
        normalization = max(normalization, max(v.x, max(v.y, v.z)));
        vertices.push_back(v);
        i++;
      }

      /*Apply vertex normalization*/
      for (Vector3D &vertex : vertices) {
        vertex.x *= 1.0/(normalization);
        vertex.y *= 1.0/(normalization);
        vertex.z *= 1.0/(normalization);
      }

      /*Add in the polymesh with id the number of vertices in it*/
      polymesh->vertices.resize(vertices.size());
      copy(vertices.begin(), vertices.end(), polymesh->vertices.begin());
      polynode.instance = polymesh;
      polymesh->id   = i;
      polymesh->name = "NAIVE_POLYGON";
      polymesh->type = Instance::POLYMESH;
      scene->nodes.push_back(polynode);
  ...
  int PointCloudParser::save( const char* filename, const SceneInfo* scene ) {
