...
namespace CGL {
  ...
  int PointCloudParser::load( const char* filename, Scene* scene ) {
    ifstream in( filename );
    if ( !in.is_open() ) {
      return -1;
    } in.close();
    ...
    camera->type = CAMERA;
    camera->position = Vector3D(10,  10,  10); 
  ...
  int PointCloudParser::save( const char* filename, const Scene* scene ) {
